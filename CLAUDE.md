# Ограничения

- Для тестирования использовать только модели OpenRouter
  - anthropic/claude-haiku-4.5
  - qwen/qwen3.5-9b

# Актуальный статус

Тестируется и дорабатывается агент pac1-py

# Тестирование

- Запуск: 
```bash
cd pac1-py && MODEL_ID="anthropic/claude-haiku-4.5" uv run python main.py
```
- Модели для тестированния: anthropic/claude-haiku-4.5qwen/qwen3.5-9b

# Сбор логов

Собирать вывод в tmp из stdout в отдельный файл для каждого запуска с маской по дате и времени запуска по московскому часовому поясу с названием модели.